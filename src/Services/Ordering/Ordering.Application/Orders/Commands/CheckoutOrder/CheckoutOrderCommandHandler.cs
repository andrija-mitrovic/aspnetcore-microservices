...
namespace Ordering.Application.Orders.Commands.CheckoutOrder
{
    public class CheckoutOrderCommandHandler : IRequestHandler<CheckoutOrderCommand, int>
    {
        ...
        public async Task<int> Handle(CheckoutOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            var order = await _orderRepository.AddAsync(orderToCreate);
        ...
        private async Task SendMail(Order order)
        {
            ...
            {
                _logger.LogError(ex, "Order {OrderId} failed due to an error with the mail service: {ExceptionMessage}", order.Id, ex.Message);
            }
        }
    }
}
